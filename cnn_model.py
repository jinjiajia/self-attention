...
class TCNNConfig(object):
    ...
    kernel_size = 3        # 卷积核尺寸###word 2 char 3
...
class TextCNN(object):
    ...
    def attention(self):
        ...
        with tf.variable_scope('add_Position_Embedding'):
            position_embedding=positional_encoding(self.input_x,self.config.embedding_dim)
            ...
            for i in range(self.config.num_blocks):
                with tf.variable_scope("num_blocks_{}".format(i)):
                    ...
                    enc = multihead_attention(keys=position_embedding,
                                              num_units=self.config.embedding_dim,
                                              num_heads=self.config.nb_head,
                                              dropout_rate=1-self.config.dropout_keep_prob,
                                              is_training=True)
                    enc = feedforward(enc, num_units=self.config.embedding_dim)
        return enc
